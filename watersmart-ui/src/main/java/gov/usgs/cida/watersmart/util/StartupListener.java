...
public class StartupListener implements ServletContextListener {
    ...
    public void contextInitialized(ServletContextEvent sce) {
        try {
            String repositoryPath = props.getProperty("watersmart.file.location") + props.getProperty(
                    "watersmart.file.location.wps.repository");
            File repositoryPathFile = new File(repositoryPath);

            String webPath = sce.getServletContext().getRealPath("/");
            //System.setProperty("watersmart.context.path", webPath);
            initializeFileRepository(sce);
        }
        catch (InterruptedException ex) {
            LOG.error(
                    "Could not create file repository directory. WPS output will not be able to dump files for retrieval. Error follows:\n{}",
                      ex.getMessage());
        }
    ...
    private File initializeFileRepository(ServletContextEvent sce) throws
        ...
        Process process = Runtime.getRuntime().exec(new String[] { "ln", "-s", repositoryPath, webPath + props.getProperty(
                    "watersmart.file.location.wps.repository") });
        process.waitFor();
        process.destroy();
